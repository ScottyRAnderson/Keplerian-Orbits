/// Grid shader provided by ogxd under MIT license
/// Source: https://github.com/ogxd/grid-shader-unity
...
Shader "Ogxd/Grid"
{
   Properties
   {
      ...
      _CutoffDistance("Cutoff Distance", Float) = 1.0
      _CutoffFade("Cutoff Fade", Float) = 1.0
   }
   SubShader
   {
      ...
      Pass
      {
         ...
         fixed4 frag (v2f i) : SV_Target
         {
            ...
            // Custom additional implementation to fade out the grid at a certain distance
            float distanceToVert = distance(_WorldSpaceCameraPos, i.worldPos);
            float interpolant = distanceToVert * _CutoffFade;
            float normalized = (interpolant - 0) / (10000 - 0);
            col = lerp(col, float4(0, 0, 0, 0), normalized);

            if (distanceToVert > _CutoffDistance) {
                col = float4(0, 0, 0, 0);
            }
